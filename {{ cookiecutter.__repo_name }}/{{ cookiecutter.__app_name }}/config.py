...
exclusive to the {{ cookiecutter.__app_name }} package. You can use the `config.py`
...
"""
# Do NOT import anything from `api` or `{{ cookiecutter.__app_name }}` packages here.
...
MODELS_PATH = os.getenv("{{ cookiecutter.__app_name|upper }}_MODELS_PATH", default=BASE_PATH / "models")
MODELS_PATH = Path(MODELS_PATH)
# Path definition for data folder
DATA_PATH = os.getenv("{{ cookiecutter.__app_name|upper }}_DATA_PATH", default=BASE_PATH / "data")
DATA_PATH = Path(DATA_PATH)

# configure logging:
# logging level across various modules can be setup via USER_LOG_LEVEL,
# options: DEBUG, INFO(default), WARNING, ERROR, CRITICAL
ENV_LOG_LEVEL = os.getenv("{{ cookiecutter.__app_name|upper }}_LOG_LEVEL", "INFO")
LOG_LEVEL = getattr(logging, ENV_LOG_LEVEL.upper())

# EXAMPLE on how to load environment variables
PARAMETER_INT = int(os.getenv("{{ cookiecutter.__app_name|upper }}_PARAMETER_INT", default="10"))
